...
void Iterator::previous(int idx){
    ...
    if (position == container->first)
        position = container->first->previous; // Remember that even though the last element in the list is really NULL, that version if last is really just a termination indicator. Last always points to a node with a real value so long as the list is not truly empty
    else
     */
